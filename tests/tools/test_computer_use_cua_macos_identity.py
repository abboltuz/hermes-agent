...
def test_private_daemon_launches_directly_off_macos():
    assert cua_backend._embedded_daemon_spawn_command(
        "/usr/local/bin/cua-driver",
        ["serve", "--socket", "/tmp/cua.sock"],
        platform="linux",
    ) == [
        "/usr/local/bin/cua-driver", "serve", "--socket", "/tmp/cua.sock",
    ]


def test_private_daemon_launches_signed_app_identity_on_macos(monkeypatch):
    ...
    assert checked == [app_path]
    assert command == [
        "/usr/bin/open", "-n", "-g", "-a", app_path, "--args",
        "serve", "--socket", "/tmp/cua.sock",
    ]
