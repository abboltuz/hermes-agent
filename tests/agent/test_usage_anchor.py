...
class TestAnchorInvalidation:
    ...
    def test_explicit_invalidation_sites(self):
        """Actual native and session boundaries invalidate both usage snapshots."""
        from unittest.mock import Mock
        from agent.codex_runtime import _record_codex_app_server_compaction
        from run_agent import AIAgent

        native = SimpleNamespace(_usage_anchor=object(), _turn_base_usage_anchor=object())
        assert _record_codex_app_server_compaction(native, SimpleNamespace(compacted=True))
        assert native._usage_anchor is None
        assert native._turn_base_usage_anchor is None

        agent = AIAgent.__new__(AIAgent)
        agent._usage_anchor = object()
        agent._turn_base_usage_anchor = object()
        agent._transition_context_engine_session = Mock()
