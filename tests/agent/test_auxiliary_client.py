...
class TestCodexAuxiliaryAdapterTimeout:
    ...
    def test_enforces_total_timeout_while_stream_keeps_emitting_events(self, monkeypatch):
        clock = [100.0]
        monkeypatch.setattr("agent.auxiliary_client.time.monotonic", lambda: clock[0])
        ...
        class _Stream:
            def __iter__(self):
                ...
                # A heartbeat is not meaningful summary progress. Advance the
                # clock at the stream boundary, not on implementation-specific reads.
                clock[0] += 0.06
