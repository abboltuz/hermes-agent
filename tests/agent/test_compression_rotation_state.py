...
class TestRotationChildFlushDedup:
    ...
    def test_adoption_divergence_merged_stamps_both_views_and_no_duplicate(
        ...
        db.append_message(
            parent, "user", "live question", origin_kind="human_user",
            turn_kind="prompt", trust_kind="user_authorized",
        )
    ...
    def test_rotation_stamps_anchor_source_when_reanchor_fallback_rewrote_turn(
        ...
        """Stamp the human carrier, never a stale persistence index on a todo row.

        Our typed-message boundary already excludes todo scaffolding from reanchoring;
        simulate an index captured before the transcript was rewritten instead.
        """
        ...
        loaded = db.get_messages_as_conversation(parent)
        # The captured persistence cursor may outlive an in-place rewrite.
        messages = [
        ...
        assert reanchor_current_turn_user_idx(messages, "current ask") == 1
        anchor_source = max(i for i, m in enumerate(messages) if _is_real_user_message(m))
        assert anchor_source == 1
        drifted = 2  # stale cursor, deliberately not recomputed after the rewrite
