...
def judge_goal(
    ...
    *,
    ...
    try:
        ...
        resp = call_llm(
            task="goal_judge",
            **({**route, "allow_fallback": False} if route else {}),
            messages=[
...
def run_kanban_goal_loop(
    *,
    ...
    judge_route: Optional[Dict[str, str]] = None,
    log=None,
) -> Dict[str, Any]:
    ...
    (str -> result dict, or legacy str), ``task_status_fn`` (() -> str|None), and ``block_fn``
    (reason: str -> None).

    Returns a decision dict: ``{"outcome", "turns_used", "reason"}`` where
    outcome is one of ``"completed_by_worker"``, ``"review_requested_by_worker"``,
    ``"changes_requested_by_reviewer"``, ``"blocked_budget"``,
    ``"blocked_unachievable"``, ``"blocked_by_worker"``, ``"blocked_judge_unavailable"``,
    ``"failed_provider"`` (with the original ``failure_result``), or ``"stopped"``.
    """
    ...
    judge_kwargs = {"route": dict(judge_route)} if judge_route else {}
    ...
    while True:
        ...
        # A failed provider call is not work for an LLM to evaluate. Preserve
        # the structured result for the CLI exit/dispatcher retry contract.
        if isinstance(last_result, dict) and last_result.get("failed"):
            return {
                "outcome": "failed_provider", "turns_used": turns_used,
                "reason": last_result.get("failure_reason") or "provider_error",
                "failure_result": last_result,
            }
        verdict, reason, _parse_failed, _wait, _transport_failed = judge_goal(
            goal_text, last_response, **judge_kwargs,
        )
        if _transport_failed or _parse_failed:
            # This is the continuation path, not a terminal handoff gate.
            # Keep completed work, but do not spend more worker turns trying
            # to cure an unavailable or malformed judge.
            failure = "transport" if _transport_failed else "invalid_response"
            try:
                block_fn(f"Goal judge unavailable ({failure}); worker progress retained")
            except Exception as exc:
                _log(f"kanban goal loop: block_fn failed ({exc})")
            return {"outcome": "blocked_judge_unavailable", "turns_used": turns_used,
                    "reason": failure}
        if verdict == "wait":
            verdict = "continue"
        ...
        try:
            last_result = run_turn(prompt)
            last_response = (
                last_result.get("final_response", "") if isinstance(last_result, dict)
                else last_result or ""
            )
        except Exception as exc:
