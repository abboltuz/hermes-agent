...
const withMessageId = (params: Record<string, unknown>) =>
  expect.objectContaining({ ...params, message_id: expect.any(String) })
      ...
      if (method === 'session.compress') {
        return {
          ...
          messages: [
            {
              role: 'user',
              content: 'compute-host summary',
              origin_kind: 'human_user',
              turn_kind: 'prompt',
              trust_kind: 'user_authorized',
              provenance_metadata: { message_id: 'desktop:compute-host-summary' }
            },
...
describe('usePromptActions submit / queue drain semantics', () => {
  ...
  it('pins prompt.submit to the active registry connection when the remote session row is untagged', async () => {
    ...
    expect(requestGatewayForAgent).toHaveBeenCalledWith(
      ...
      'default',
      'prompt.submit',
      withMessageId({ session_id: 'runtime-remote', text: 'continue remotely' }),
      1_800_000
    )
...
describe('usePromptActions file attachment sync', () => {
  ...
  it('passes a Windows path directly for a native Windows local backend', async () => {
    ...
    expect(calls[1]).toEqual({
      method: 'prompt.submit',
      params: withMessageId({ session_id: RUNTIME_SESSION_ID, text: '@file:data/report.txt\n\nsummarize' })
    })
  })
})
...
describe('usePromptActions sleep/wake session recovery', () => {
  ...
  it('lets the durable route replace a stale selected session and runtime before submit', async () => {
    ...
    expect(requestGateway).toHaveBeenCalledWith(
      'prompt.submit',
      withMessageId({ session_id: RECOVERED_SESSION_ID, text: 'stay in the routed profile session' }),
      1_800_000
    )
  })
  ...
  it('submits directly when the routed stored session already owns the live runtime', async () => {
    ...
    expect(requestGateway).toHaveBeenCalledWith(
      'prompt.submit',
      withMessageId({ session_id: RECOVERED_SESSION_ID, text: 'normal follow-up' }),
      1_800_000
    )
  })
  ...
  it('never falls through to session.create or a stale runtime when routed-session recovery fails', async () => {
    ...
    expect(requestGateway).toHaveBeenCalledWith(
      'prompt.submit',
      withMessageId({ session_id: RECOVERED_SESSION_ID, text: 'retry after recovery' }),
      1_800_000
    )
  })
...
describe('usePromptActions new-chat first-send delivery (#63078)', () => {
  ...
  it('delivers the first message of a new chat through the intentional route transition (#62562)', async () => {
    ...
    expect(calls).toEqual([
      {
        method: 'prompt.submit',
        params: withMessageId({ session_id: NEW_RUNTIME_ID, text: 'first message of a new chat' })
      }
    ])
  })
...
describe('usePromptActions busy-gateway churn tolerance (#64327)', () => {
  ...
  it('keeps an explicit background queue target isolated from foreground routed recovery (#90428)', async () => {
    ...
    expect(calls).toEqual([
      ...
      {
        method: 'prompt.submit',
        params: withMessageId({ session_id: QUEUED_RUNTIME_ID, text: 'queued prompt for C', queued: true })
      }
    ])
