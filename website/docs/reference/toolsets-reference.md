...
| `desktop_ui` | `annotate_preview`, `close_terminal`, `desktop_preview`, `drive_preview`, `focus_pane`, `react_to_message`, `read_terminal`, `read_window_below`, `setup_mcp`, `tour` | Affordances that act on the Hermes desktop app itself — read/close the embedded terminal pane, open/read/close/interact with/annotate the in-app browser, identify the OS window behind the app, reveal a pane, react to a message, configure MCP, run a guided tour (highlight + narrate UI elements in the app or the preview pane). Enabled for sessions whose source is the desktop app, whichever backend it's connected to (local, SSH, URL, or Hermes Cloud). Never present on CLI, TUI, messaging, or cron sessions. |
