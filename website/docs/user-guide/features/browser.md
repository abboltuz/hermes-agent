...
Hermes closes only the harness runtimes it created, after `browser.inactivity_timeout` (120 seconds by default) and at process shutdown. It first closes the exact tab recorded for that task/session, then requires Browser Harness 0.1.10's clean-shutdown acknowledgement and verifies the daemon PID and runtime endpoints. With Harness 0.1.9, local/CDP daemons use an authenticated shutdown acknowledgement plus the same verification; Hermes refuses to report successful cleanup for a 0.1.9 Browser Use Cloud daemon because that version cannot confirm cloud stop, profile persistence, or billing cleanup. Orphan recovery uses crash-released process claims and verified owner markers; it never scans for or kills unrelated browser processes or closes unrelated tabs. Advanced operators who set `BH_RUNTIME_DIR` or `BH_TMP_DIR` explicitly retain full ownership of that runtime, and Hermes passes those paths through without registering, stopping, or reaping them.
