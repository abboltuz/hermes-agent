...
Hermes closes only the harness runtimes it created, after `browser.inactivity_timeout` (120 seconds by default) and at process shutdown. It uses Browser Harness's supported `--reload` boundary and verifies an owner marker before recovering an orphan from a previous Hermes process; it never scans for or kills unrelated browser processes. Advanced operators who set `BH_RUNTIME_DIR` or `BH_TMP_DIR` explicitly retain full ownership of that runtime, and Hermes passes those paths through without registering, reloading, or reaping them.
