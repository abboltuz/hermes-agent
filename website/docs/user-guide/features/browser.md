...
**Concurrent sessions:** `browser_exec` accepts a `session=<name>` argument that creates an explicit logical browser identity. Every distinct name gets its own harness runtime (IPC socket, log, and state), and on cloud backends its own browser; deliberately reusing the same name from another task reconnects to that same logical session and serializes its harness calls. Omitting `session` uses a task-scoped default runtime, so unrelated simultaneous chats do not share harness state. Every call also receives a unique persistent screenshot directory; a later or parallel call cannot overwrite the image path returned by an earlier call.

Hermes closes only the harness runtimes it created, after `browser.inactivity_timeout` (120 seconds by default) and at process shutdown. It first closes the exact tab recorded for that task/session, then verifies the daemon PID and runtime endpoints. For Browser Use Cloud, Hermes records Browser Harness 0.1.10's strict-cleanup capability against the exact daemon PID when that daemon is created; updating the CLI does not silently upgrade an already-running daemon's cleanup authority. A cloud daemon without that proof fails closed because stop, profile persistence, and billing cleanup cannot be confirmed. Harness 0.1.9 local/CDP daemons remain compatible through an authenticated shutdown acknowledgement plus the same PID/endpoint verification. Confirmed runtimes and Harness's bounded cleanup directory are retired without dropping their ownership marker early. Orphan recovery uses crash-released process claims and verified owner markers; it never scans for or kills unrelated browser processes or closes unrelated tabs. Advanced operators who set `BH_RUNTIME_DIR` or `BH_TMP_DIR` explicitly retain full ownership of that runtime, and Hermes passes those paths through without registering, stopping, or reaping them.

Native JavaScript dialogs are explicit in Browser Use mode. If a trusted CDP click opens an `alert`, `confirm`, or `prompt`, `click_at_xy` returns a `dialog_pending` result instead of waiting forever for `mouseReleased`. Inspect the dialog and then use `cdp('Page.handleJavaScriptDialog', accept=True/False, promptText='...')`. If the CDP input channel stops responding without a dialog, Hermes reports `HERMES_BROWSER_INPUT_WEDGED` and attempts to stop that exact Hermes-owned harness runtime.
