...
separate browser tools (or `browser_exec` when that backend is active), not
`computer_use`. The former
...
launched for bounded or unrestricted access. One Hermes conversation cannot
change another runtime's mode or grants. On macOS these private runtimes launch
through the verified `CuaDriver.app` bundle so Accessibility and Screen
Recording remain attached to Cua's stable TCC identity.
