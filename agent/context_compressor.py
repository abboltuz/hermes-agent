...
from agent.message_provenance import (
    OriginKind, TurnKind, SEMANTIC_FIELDS, classify_legacy_message, decode_message_provenance,
    display_actor, is_actionable_continuation, is_human_intent,
)
...
def _has_non_human_provenance(message: Dict[str, Any]) -> bool:
    """Respect our typed-message boundary without changing upstream's legacy heuristics."""
    if any(field in message for field in SEMANTIC_FIELDS):
        # Explicit unknown/invalid claims stay non-human; only an unstamped
        # legacy API caller falls through to upstream's content heuristics.
        return not is_human_intent(message)
    provenance = classify_legacy_message(message)
    return provenance.origin_kind != OriginKind.LEGACY_UNKNOWN and not is_human_intent(message)
...
def _prune_stale_reasoning_replay(messages: List[Dict[str, Any]]) -> int:
    ...
    last_user_idx = next((
        ...
        and (not _has_non_human_provenance(messages[i]) or is_actionable_continuation(messages[i]))
    ), -1)
    if last_user_idx < 0:
...
def _build_verbatim_user_section(turns: List[Dict[str, Any]]) -> str:
    ...
    for msg in reversed(turns):
        if msg.get("role") != "user" or _has_non_human_provenance(msg):
            continue
...
def _collect_protected_skill_names(messages: List[Dict[str, Any]], prune_boundary: int) -> set[str]:
    ...
    tail_user_texts = [
        ...
        if m.get("role") == "user" and not _has_non_human_provenance(m)
...
def _strip_historical_media(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def _newest(role: str, has_images) -> int:
        hits = (
            ...
            and (role != "user" or not _has_non_human_provenance(m))
        )
...
class ContextCompressor(MicroCompactionMixin, ContextEngine):
    ...
    def _serialize_for_summary(self, turns: List[Dict[str, Any]]) -> str:
        ...
        for msg in turns:
            ...
            if role == "user":
                ...
                if provenance.turn_kind in {TurnKind.NOTIFICATION, TurnKind.DELIVERY_MIRROR}:
                    continue
                if provenance.origin_kind != OriginKind.LEGACY_UNKNOWN or any(field in msg for field in SEMANTIC_FIELDS):
    ...
    def _fallback_anchors(self, turns_to_summarize: List[Dict[str, Any]]) -> Dict[str, list[str]]:
        ...
        for msg in turns_to_summarize:
            ...
            if role == "user" and _has_non_human_provenance(msg):
                turn_label = display_actor(msg).upper()
            if turn_text.strip():
...
Write only the summary body. Do not include any preamble or prefix."""
    ...
    def _is_synthetic_compression_user_turn(cls, message: Any) -> bool:
        ...
        if not isinstance(message, dict) or message.get("role") != "user":
            return False
        if cls._is_context_summary_message(message) or _has_non_human_provenance(message):
            return True
    ...
    def _is_actionable_user_turn(cls, message: Any) -> bool:
        ...
        if message.get("display_kind") or cls._is_context_summary_message(message) or _has_non_human_provenance(message):
            return False
...
def reference_handoff_would_drive_next_model_call(messages: Optional[List[Dict[str, Any]]]) -> bool:
    ...
    for message in messages[last_driving_handoff + 1 :]:
        ...
        if (
            ...
            or ContextCompressor._is_inflight_task_turn(message)
