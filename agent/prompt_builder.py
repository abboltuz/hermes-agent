...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    if is_macos:
        offscreen_line = (
            "- If an element is behind another window, cua-driver can still "
            "drive it without raising it. Windows on another Space may not "
            "be discoverable through the current capture selector.\n\n"
        )
    elif is_windows:
        offscreen_line = (
    ...
    return (
        ...
        "(default). You get a plain screenshot plus a separately numbered "
        "AX-tree index listing role, label, and bounds for each element. "
        "The numbers are not drawn over the screenshot.\n"
        ...
        "4. `capture_after=true` is fresh visual/AX evidence, not proof that "
        "the requested task postcondition holds. For a bounded deterministic "
        "check, call `action='verify_state'` with the exact `pid`, "
        "`window_id`, and 1–8 `expect` predicates. Only `status: 'satisfied'` "
        "proves those predicates; `unsatisfied` means continue safely, and "
        "`unknown` means capture/inspect fresh state and never declare done.\n"
        "5. Use `action='list_windows'` for a truthful native-window inventory. "
        "It defaults to on-screen windows. `on_screen_only=false` requires an "
        "exact `pid` so off-Space window titles are not listed globally.\n"
        "6. `action='launch_app'` requires exactly one `bundle_id` or app "
        "name. Hermes never requests focus/raise; app may self-activate. "
        "Inspect the returned zero/one/many windows and "
        "capture an exact target separately.\n"
        "7. `action='set_window_frame'` takes exact `pid`, `window_id`, and "
        "`frame={x,y,width,height}` in desktop coordinates. It does not focus "
        "the window. A confirmed actuator result still requires a separate "
        "`verify_state` bounds predicate before declaring the layout done.\n\n"
        ...
        "- `effect: 'confirmed'` (or `verified: true`) — the actuator effect "
        "was read back, but the task postcondition still needs fresh "
        "verification. Never repeat successful input blindly.\n"
        "- `effect: 'partial'` or `'unverifiable'` — some or all input was "
        "delivered without complete proof. Get fresh state and check it before "
        "any retry; an escalation hint does not override this rule.\n"
        "- `effect: 'refused'`, `'suspected_noop'`, or a structured refusal "
        "such as `code: 'background_unavailable'` — escalation is allowed. "
        "Follow `escalation.target` when present:\n"
        "  - `'pixel'` → re-issue addressing the target by `coordinate=[x,y]` "
        "read off the screenshot instead of `element`.\n"
        "  - `'page'` → inspect or act on page content with the separate "
        "browser toolset (or `browser_exec` when that backend is active). "
        "This is a route recommendation, not a computer_use action.\n"
        ...
        "computer_use exposes desktop actions only. Use native capture/input "
        "for browser chrome, OS permission prompts, and native dialogs; use "
        "the separate browser tools for semantic page content. Do not invent "
        "`cua_browser_*` actions or profile-grant configuration keys.\n\n"
        ...
        "not input.\n\n"
        "## Safety\n"
        "- Do NOT click OS permission dialogs, payment UI, or 2FA the user "
        "did not ask for. If you encounter one unexpectedly, stop and ask.\n"
        ...
        "guess credentials, and do not type payment card numbers unless that "
