...
def init_agent(
    ...
    # clamped 2.5%/10K-25K tail with recovery-pointer machinery; explicit
    # "legacy" retains the older threshold-proportional tail.
    compression_tail_mode = str(
        _compression_cfg.get("tail_mode", "lean")
    ).strip().lower()
